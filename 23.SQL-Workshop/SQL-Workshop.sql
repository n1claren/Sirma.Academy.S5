...
payment_date DATE,
amount DECIMAL(12,2),
payment_method NVARCHAR(20) CHECK (payment_method IN ('Bank Transfer', 'Credit Card',
'Cash')),
...
INSERT INTO Clients (name, contact_email, phone, address) 
VALUES
...
INSERT INTO Invoices (client_id, issue_date, due_date, amount, tax_rate, vat, discount,
currency, status) 
VALUES
...
INSERT INTO Transactions (supplier_id, employee_id, transaction_date, amount,
...
INSERT INTO Payments (invoice_id, payment_date, amount, payment_method, bank_name,
transaction_id) 
VALUES
...
SELECT c.name AS client_name, i.invoice_id, i.issue_date, i.due_date, i.amount
FROM Invoices i
...
SELECT MAX(amount) AS highest_invoice, MIN(amount) AS lowest_invoice
FROM Invoices;

-- -- -- Reports -- -- --

-- Create a view showing invoices with their corresponding payments
CREATE VIEW InvoicesWithPayments AS
SELECT i.invoice_id, i.client_id, i.issue_date, i.due_date, i.amount, p.payment_date, p.amount AS payment_amount
FROM Invoices i
LEFT JOIN Payments p ON i.invoice_id = p.invoice_id;

SELECT * FROM InvoicesWithPayments

-- Create a report showing monthly revenue for the year 2024
SELECT MONTH(p.payment_date) AS month, SUM(p.amount) AS monthly_revenue
FROM Payments p
JOIN Invoices i ON p.invoice_id = i.invoice_id
...
CREATE PROCEDURE GetInvoicesByClientID @client_id INT
AS
BEGIN
    SELECT *
    FROM Invoices
...
EXEC GetInvoicesByClientID @client_id = 1;

-- Generate a report showing the top 3 clients who have paid the most
SELECT TOP 3 c.name AS client_name, SUM(p.amount) AS total_paid
FROM Payments p
JOIN Invoices i ON p.invoice_id = i.invoice_id
JOIN Clients c ON i.client_id = c.client_id
GROUP BY c.name
